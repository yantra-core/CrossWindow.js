function calculateEntryPosition(direction, position, bestWindowId) {
  ...
  const buffer = 100; // any small values should work, without buffer the entity may get stuck in teleportation loop
...
function prepareBestWindowResponse(cw, bestWindowId, direction, entityData) {
  return {
    ...
    postMessage: (data) => {
      const newEntryPosition = calculateEntryPosition(direction, data.position, bestWindowId);
