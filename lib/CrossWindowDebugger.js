...
export default class CrossWindowDebugger {
  ...
  removeDebugContainer(windowId) {
    const containerId = `windowBox_${windowId}`; // Assuming container IDs follow this pattern
  ...
  updateUI() {
    setInterval(() => {
      ...
      if (this.config.showOtherWindows) {
        const currentWindowIds = Object.keys(currentWindows);
  
        // Remove UI elements for windows that are no longer active
        Object.keys(this.displayedWindows).forEach(displayedWindowId => {
          if (!currentWindowIds.includes(displayedWindowId)) {
            this.removeDebugContainer(displayedWindowId);
          }
        });
        ...
        currentWindowIds.forEach(windowId => {
          ...
          this.displayedWindows[windowId] = true; // Mark this window as displayed
        });
  
        // Update the tracking object to reflect the current set of windows
        this.displayedWindows = currentWindowIds.reduce((acc, id) => {
          acc[id] = true;
          return acc;
        }, {});
      }
